...
const ParcelStatistics: React.FC = () => {
  ...
  const dispatched =
    data?.filter((parcel) => parcel.parcelStatuses[0].status === "Dispatched")
      .length || 0;
  const delivered =
    data?.filter((parcel) => parcel.parcelStatuses[0].status === "Delivered")
      .length || 0;
  const pending =
    data?.filter((parcel) => parcel.parcelStatuses[0].status === "Pending")
      .length || 0;
