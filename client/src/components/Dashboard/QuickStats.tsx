...
const QuickStats = () => {
  ...
  const deliveredParcels =
    data?.filter((parcel) => parcel.parcelStatuses[0].status === "Delivered").length || 0;
  const dispatchedParcels =
    data?.filter((parcel) => parcel.parcelStatuses[0].status === "Dispatched").length ||
    0;
  const pendingParcels =
    data?.filter((parcel) => parcel.parcelStatuses[0].status === "Pending").length || 0;
