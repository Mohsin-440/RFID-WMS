...
import { LuPackage, LuPackagePlus, LuWarehouse } from "react-icons/lu";
...
type MenuDatum = {
  ...
  items: {
    name: string;
    icon: ReactElement;
    link: ((warehouseId: string) => string) | string;
    visible: Role[];
  }[];
}

const menuData: MenuDatum[] = [
  {
    title: "MENU",
    items: [
      {
      ...
      },
      {
        name: "Add Parcel",
        icon: <LuPackagePlus />,
        link: (warehouseId) => `/${warehouseId}/parcels/add`,
        visible: ["Manager", "CounterMan", "Admin"],
      },
      {
        name: "All Parcels",
        icon: <LuPackage />,
      ...
      },
      {
        ...
        link: (warehouseId) => `/${warehouseId}/registration`,
        visible: ["Admin"],
      },
      {
        name: "All Users",
        icon: <RiUserLine />,
        link: (warehouseId) => `/${warehouseId}/users`,
        ...
        <div className="flex flex-col gap-1" key={section.title}>
          ...
          {section.items.map((item) => {
            ...
            if (typeof item.link === "function")
              link = item.link(params.warehouseId)

            else
              link = item.link

            return item.visible ? (
              <Authorization roles={item.visible as Role[]} key={item.name} navigate={false}>
                <Link
                  href={link}
                  onClick={
