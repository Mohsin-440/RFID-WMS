...
type TagWithParcel = Props["tags"][0] & { parcel: ParcelDetailsWithStatus }

export const tagsMonitored = async (baseIo: SocketServer, socket: Socket, props: Props) => {
    try {
        ...
        for (const sessionSocketId of sessionSocketIds) {
            await new Promise(async (resolve) => {
                ...
                const tagsWithParcel: TagWithParcel[] = []
                ...
                baseIo.to(sessionSocketId).emit("server-to-client:tags-monitored", tagsWithParcel)
                resolve(true)
            })
        }
